...
class CommandAutoSuggest(AutoSuggest):
    ...
    def get_suggestion(
        ...
        # Only suggest the first argument for MCP prompts (not for local commands like /help)
        if len(parts) == 1:
            cmd = parts[0]
...
class UnifiedCompleter(Completer):
    def __init__(self):
        ...
        # Local slash commands that are handled in CliChat, not by MCP prompts
        self.local_commands: List[str] = [
            "help",   # new local command
            "docs",
            "import",
            "info",
            "rename",
    ...
    def get_completions(self, document, complete_event):
        ...
        # Slash commands and their args
        if text.startswith("/"):
            parts = text[1:].split()

            # Completing the command name: /<cmd>
            if len(parts) <= 1 and not text.endswith(" "):
                cmd_prefix = parts[0] if parts else ""

                # 1) Local commands (e.g. /help)
                ...
                # 2) MCP prompts (e.g. /summarize, /format, ...)
            ...
            # Completing the first argument for MCP prompts (doc id, etc.)
            ...
            # Completing subsequent args that look like doc IDs
            if len(parts) >= 2:
                doc_prefix = parts[-1]
    ...
    def __init__(self, agent: CliChat):
        ...
        def _(event):
            ...
            if text.startswith("/"):
                ...
                if len(parts) == 1:
                    # After typing /cmd<space>, suggest arguments (doc ids)
                    buffer.start_completion(select_first=False)
                elif len(parts) == 2:
                    arg = parts[1]
                    if (
